...
import type { RepairEstimate } from '../types/domain';
 ...
 */
export async function getRepairEstimate(input: EstimateInput): Promise<RepairEstimate> {
  ...
  const repairDesc = input.freeformDescription
    ...
    : `Requested repair: ${input.repairLabel}${
        input.baselineHours ? ` (typical shop baseline: ${input.baselineHours} labor hours)` : ''
      }`;
  ...
  const raw = await getEstimateViaToolUse(modelId, basePrompt, messages).catch(() =>
    getEstimateViaJsonPrompt(modelId, basePrompt, messages)
  );

  const laborCost = round2(raw.estimatedHours * input.shopRatePerHour);
...
async function getEstimateViaToolUse(modelId: string, basePrompt: string, messages: Message[]): Promise<RawEstimate> {
...
function validateRawEstimate(value: unknown): RawEstimate {
  const v = value as Partial<RawEstimate> | null;
  if (
    !v ||
